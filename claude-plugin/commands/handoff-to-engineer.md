...
Read `.agents/state.json` for project mode (check `mode` — if `"mvp"`, skip non-essential quality checks).

**Step 4: Execute.**
Implement the task following your protocol. When done:
- Commit the work
- Update `.agents/state.json` task status to `done`
- Report back: what was built, what files changed, test results
