...
### Added
- **`attacca-init` / `validate-idea` skill** — falsification-first product/feature idea validation: decomposes an idea into load-bearing assumptions, pre-registers kill criteria before gathering evidence, runs cost-ordered disqualifiers, keeps a cited evidence ledger, red-teams the idea in an independent context, and issues a traceable verdict with one concrete next experiment.
...
### Changed
- **`.claude/skills/` + `.github/skills/`** — Removed 33 marketing/CRO skills (copywriting, SEO, CRO variants, email sequences, paid ads, etc.) from core boilerplate. These ship as a separate addon package; recoverable from git at commit `ee5af68`.
- **`.kiro/`** — Removed full Kiro IDE port (hooks, skills, steering) from core boilerplate. Ships as a separate addon package; recoverable from git at commit `ee5af68`.
- **`.github/workflows/deploy-website2.yml`** — Removed stale deployment workflow.

### Why
Core template was bloated for projects that don't need marketing automation or Kiro IDE support. Splitting to addons lets teams opt in to only what they use, without the noise in the base install.
