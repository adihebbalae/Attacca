---
name: quality-gate
description: "Pre-push quality gate checklist. Use when: before any git push, before opening a PR, after implementing a full feature. Runs lint, type-check, tests, and a security scan as a single sequential gate. Fails fast — does not proceed past a failing stage."
---

<!-- Delete when: native Claude Code local pre-commit hook integration with language-specific linters/formatters is built-in -->
