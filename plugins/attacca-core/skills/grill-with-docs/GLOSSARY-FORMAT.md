...
The domain glossary. Distinct from `CONTEXT.md`, which is project orientation
(see `docs/CONTEXT-SCHEMA.md`) — one filename, one schema, so a file's name
tells you what's inside it without opening it.
