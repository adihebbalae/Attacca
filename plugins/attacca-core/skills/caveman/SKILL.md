---
name: caveman
description: "Token compression skill. Use when: context window is filling up, on long-running multi-session tasks, when user needs faster/cheaper agent responses, or when working in bandwidth-constrained environments. Reduces output tokens 65–75% by switching to telegraphic prose. Three intensity levels: lite, full, ultra."
---

<!-- Delete when: native Claude Code model selection for token budget optimization is available -->
...
When writing to documentation files or state outputs, apply `full` mode:
...
- Target: outputs ≤ 40 lines, summaries ≤ 25 lines
...
1. User's explicit request: "use caveman lite/full/ultra", "compress output", or "save tokens"
2. Implicit: if context >60% and no explicit level, default to `full`
3. Context signals: long-running tasks, multi-file edits, large codebases
