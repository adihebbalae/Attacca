...
description: "Runbooks and decision frameworks for production incidents and emergencies. Use when: service is down, critical feature broken, data loss risk, security breach detected. Covers triage, classification, symptom-to-fix matching, and incident logging."
---

<!-- Delete when: native Claude Code incident severity classifier with playbook recommendations UI is available -->
...
**DO NOT attempt to patch and deploy yourself.** Invoke the security-auditor subagent immediately with affected file paths.

**Containment**: Execute the immediate steps (rotate secrets, block IPs) while security audit is underway.
...
Every incident MUST generate a log file at `docs/postmortems/<timestamp>-<slug>.md`:

```markdown
# Incident: <title>
**Date**: <YYYY-MM-DD HH:MM UTC> | **Severity**: SEV 1 | **Resolved by**: [responder name]
...
## Follow-Up Actions
- [ ] Security audit of the fix (if auth/input/data touched)
...
5. **Escalate security breaches immediately** — invoke security-auditor subagent with file paths
