# Security Classifier Protocol (v3.11.0+)
...
3. **Diff size and scope within thresholds:**
   - Lines changed: < 300 (default; configurable via `.agents/security-classifier.config.json`)
...
6. **No BDR present** — Legacy commits without a Business/Decision/Rationale header. Treat as COMPLEX for v3.11.0 (may relax in future versions with better heuristics).
...
Projects may create an **optional** file `.agents/security-classifier.config.json`:
...
The per-PR Security agent outputs a structured audit report to `.agents/audits/<pr-id>.md` that includes:
- Branch name
...
The Manager reads this report and routes the PR accordingly:
...
When a project enables auto-land via `.agents/security-classifier.config.json` with `"auto_land_simple": true`:

1. Per-PR Security audit produces a SIMPLE classification
2. Manager verifies the classification
3. If SIMPLE and auto-land enabled: Manager runs `git merge --no-ff <branch> -m "merge: ..."` and records in state.json
4. If COMPLEX or auto-land disabled: Manager routes to human review queue

See the Manager protocol's "Per-PR Review (Parallel Mode)" section for the full auto-land flow and safety properties.

## Future Enhancements (not in v3.11.0)
...
- **Merge coordinator** — Schedule optimal landing order when multiple SIMPLE PRs are in flight (deferred to v3.12+)
